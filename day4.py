...
def _get_diagonals(block: str, side_len: int) -> List[str]:
    ...
    for d in range(side_len):
        ...
        for x in range(d + 1):
            ...
            line = line + block[d * side_len - (x * (side_len - 1))]
        lines.append(line)
    ...
    for d in range(side_len - 1, 0, -1):
        ...
        for y in range(d):
            ...
            line = (
                line
                + block[
                    ((side_len - 1) * side_len) + (side_len - d) - (y * (side_len - 1))
                ]
            )
        lines.append(line)
...
def _get_other_diagonals(block: str, side_len: int) -> List[str]:
    ...
    for d in range(side_len - 1, -1, -1):
        ...
        for x in range(side_len - d):
            ...
            line = line + block[side_len - (side_len - d) + (x * (side_len + 1))]
        lines.append(line)
    ...
    for d in range(1, side_len):
        ...
        for y in range(side_len - d):
            ...
            line = line + block[(d * side_len) + (y * (side_len + 1))]
        lines.append(line)
